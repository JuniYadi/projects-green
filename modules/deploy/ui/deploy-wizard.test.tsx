...
describe("DeployWizard", () => {
  beforeEach(() => {
    ...
    globalThis.fetch = mock((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      const owners = Array.from(
        new Set(githubRepositories.map((repo) => repo.owner))
      ).map((owner) => ({
        id: owner,
        name: owner,
      ...
      return Promise.resolve(
        new Response(JSON.stringify({ ok: true, items, owners }), {
          status: 200,
          headers: { "content-type": "application/json" },
        })
      )
  ...
  it("renders owner from API owners when repos are empty", async () => {
    ...
    await waitFor(() => {
      expect(view.getByRole("button", { name: /my-account/i })).toBeTruthy()
    })

    expect(view.queryByText(/No accounts found/i)).toBeNull()
