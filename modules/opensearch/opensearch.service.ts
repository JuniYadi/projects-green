...
function buildOpenSearchQuery(filters: OpenSearchSearchFilters): Record<string, unknown> {
  const must: unknown[] = []
  ...
  if (filters.containerName) {
    must.push({ term: { "kubernetes.container_name.keyword": filters.containerName } })
  }
  ...
  if (filters.searchQuery) {
    const searchableColumns = [
      "message", "level", "ip", "forwarded_ip", "status", "method", "uri",
      "pod", "container", "namespace", "host", "user_agent",
    ]
...
const PINO_LEVEL_MAP: Record<string | number, string> = {
  10: "trace", 20: "debug", 30: "info", 40: "warning", 50: "error", 60: "fatal",
...
function formatLogLevel(level: string | number | undefined): string {
  ...
  const numeric = typeof level === "string" && !isNaN(Number(level)) ? Number(level) : null
  if (numeric !== null) {
...
function formatSearchResults(
  ...
  return hits.map((hit) => {
    ...
    const reqRaw = (raw["req"] ?? src["req"]) as Record<string, unknown> | undefined
    const resRaw = (raw["res"] ?? src["res"]) as Record<string, unknown> | undefined
    ...
    let httpStatus = ""
    if (typeof statusCodeRaw === "number" && statusCodeRaw >= 100 && statusCodeRaw < 600) {
      httpStatus = String(statusCodeRaw)
    } else if (typeof statusCodeRaw === "string" && /^\d{3}$/.test(statusCodeRaw)) {
    ...
    if (!method && !uri) {
      ...
      const httpMatch = msg.match(/\b(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\s+(\S+)/i)
      if (httpMatch) {
    ...
    const responseTimeRaw =
      (raw["responseTime"] as number) ??
      (src["responseTime"] as number) ??
      null

    const levelName = (src["level_name"] ?? raw["level_name"]) as string | undefined
    ...
    if (isHttpLog && (genericMsgs.includes(message.toLowerCase().trim()) || !message)) {
      message = `${method} ${uri}`
    }

    const reqHeaders = (reqRaw?.["headers"] ?? {}) as Record<string, unknown>
    const forwardedFor = (reqHeaders["x-forwarded-for"] as string) ?? (src["http_x_forwarded_for"] ?? raw["http_x_forwarded_for"] ?? "") as string
    ...
    return {
      ...
      container: (k8s["container_name"] ?? src["container_name"]) as string ?? "",
      ...
      ip:
        ...
        (src["remote_addr"] ?? src["ip"] ?? raw["remote_addr"] ?? raw["ip"]) as string ?? "",
      ...
      user_agent:
        ...
        (src["http_user_agent"] ?? src["user_agent"] ?? raw["http_user_agent"] ?? raw["user_agent"]) as string ?? "",
      raw: src,
    }
  })
}
...
function getTimelineInterval(timeRange: string = "1h"): string {
  switch (timeRange) {
    case "15m": case "1h": return "1m"
    case "6h": case "24h": return "1h"
    case "7d": return "1d"
    default: return "1h"
  }
}
...
export class OpenSearchService {
  ...
  static fromRegion(regionCode: string): OpenSearchService {
    return new OpenSearchService(OpenSearchClient.forRegion(regionCode), regionCode)
  }

  async searchLogs(filters: OpenSearchSearchFilters): Promise<OpenSearchSearchResult> {
    ...
    try {
      ...
      const result = await this.client.search(indexPattern, body, filters.size ?? 50, filters.from ?? 0)

      return {
        success: true,
        data: formatSearchResults(result.hits.hits) as unknown as OpenSearchLogEntry[],
  ...
  async getLogCounts(filters: OpenSearchSearchFilters): Promise<OpenSearchLogCounts> {
    const indexPattern = buildIndexPattern(
      filters.stackSlugs ?? [],
      filters.startDate,
      filters.endDate
  ...
  async getAvailableFields(stackSlugs: string[]): Promise<OpenSearchIndexFields> {
    ...
    try {
      ...
      for (const data of Object.values(mapping)) {
        const mappingProps = (data as { mappings?: { properties?: Record<string, { type: string }> } }).mappings?.properties
        if (mappingProps) {
