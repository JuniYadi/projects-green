...
import { MINIMUM_BALANCE_WARN_IDR, roundCpu, roundMem } from "./constants"
import {
...
export class BalanceGateService {
  ...
  calcPaygCost(pricing: PricingLookup, mcpu: number, memMb: number): Decimal {
    ...
    if (pricing.unitRateCpu) {
      total = total.plus(new Decimal(roundCpu(mcpu)).times(pricing.unitRateCpu))
    }
