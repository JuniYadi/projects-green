...
describe("AppManifestBuilder", () => {
  it("generates full manifest with ConfigMap, Secret, Deployment, HPA", () => {
    const manifest = new AppManifestBuilder()
      ...
      .addConfigMapData({ DATABASE_URL: "postgres://localhost", CACHE_TTL: "3600" })
    ...
    const configMap = manifest.resources.find((r) => r.kind === "ConfigMap")! as unknown as KubernetesConfigMap
    expect(configMap.metadata.name).toBe("my-app-config")
    expect(configMap.data).toEqual({ DATABASE_URL: "postgres://localhost", CACHE_TTL: "3600" })

    const secret = manifest.resources.find((r) => r.kind === "Secret")! as unknown as KubernetesSecret
  ...
  it("generates manifest with volume mounts", () => {
    ...
    expect(volumes).toContainEqual({ name: "app-config", configMap: { name: "app-config" } })
