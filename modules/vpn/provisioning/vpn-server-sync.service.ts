    ...
    try {
      ...
      for (const subscription of subscriptions) {
        ...
        for (const protocol of missing) {
          ...
          // ponytail: audit before dispatch — order is intentional for audit completeness
          await logAuditEvent({
            serverId,
            subscriptionId: subscription.id,
      ...
      await logAuditEvent({
        serverId,
        correlationId,
        action: "SYNC_PROTOCOLS_FAILED",
        status: "FAILED",
        message: `Sync failed for server ${serverId}: ${message}`,
        details: { error: message },
      }).catch(() => {
        // ponytail: swallow audit failures in the error handler
        console.error(
          `[vpn-server-sync] failed to log SYNC_PROTOCOLS_FAILED for server=${serverId}`
        )
      })
      throw error
    }
