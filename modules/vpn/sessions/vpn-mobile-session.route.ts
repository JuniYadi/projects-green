    ...
    new Elysia()
      ...
      .post(
        ...
        async ({ body, request, set }) => {
          ...
          if (
            !device ||
            device.subscriptionId !== serverAccount.subscriptionId
          ) {
            set.status = 403
            return {
              error: {
        ...
        },
        {
          detail: {
            ...
            description:
              "Create a new VPN session when the mobile device connects to a server.",
            security: [{ bearerAuth: [] }],
          },
          body: t.Object({
      ...
      .post(
        ...
        async ({ params, request, set }) => {
          ...
          const session = await service.findById(
            params.id,
            auth.mobileAuth.organizationId
          )
          if (!session) {
            set.status = 404
            return {
        ...
        },
        {
          detail: {
            ...
            description:
              "Send a heartbeat ping for an active VPN session. Updates lastPingAt timestamp.",
            security: [{ bearerAuth: [] }],
          },
      ...
      .patch(
        ...
        },
        {
          detail: {
            ...
            description:
              "Close a VPN session and submit traffic data on disconnect.",
            security: [{ bearerAuth: [] }],
          },
          params: t.Object({ id: t.String({ minLength: 1 }) }),
          body: t.Object({
      ...
      .get(
        ...
        async ({ query, request, set }) => {
          ...
          const sessions = result.sessions.map((s) => toSessionDetailDTO(s))
        ...
        },
        {
          detail: {
            ...
            description:
              "List VPN sessions with optional filters by status, server, or device. Supports cursor-based pagination.",
            security: [{ bearerAuth: [] }],
          },
          query: t.Object({
      ...
      .get(
        ...
        },
        {
          detail: {
            ...
            description:
              "Get dashboard session statistics grouped by server and subscription.",
            security: [{ bearerAuth: [] }],
          },
          response: {
            200: t.Object({
