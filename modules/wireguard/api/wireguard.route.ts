...
async function requireAdmin(): Promise<{ ok: true; orgId: string } | { ok: false; status: number; body: unknown }> {
  const auth = await withAuth()
  if (!auth.user) {
    return { ok: false, status: 401, body: { error: "UNAUTHORIZED", message: "Not signed in" } }
  }
  if (!auth.organizationId) {
    return { ok: false, status: 403, body: { error: "FORBIDDEN", message: "No organization" } }
  }
  const roles = new Set([auth.role, ...(auth.roles ?? [])].filter(Boolean))
  const isAdmin = ["admin", "owner", "super_admin"].some((r) => roles.has(r))
  if (!isAdmin) {
    return { ok: false, status: 403, body: { error: "FORBIDDEN", message: "Admin role required" } }
  }
...
export const wireguardRoutes = new Elysia({ prefix: "/portal/vpn/wireguard" })
  .get("/peers", async ({ set }) => {
    ...
    try {
      ...
      set.status = 503
      return { error: "SSH_CONNECTION_FAILED", message: err instanceof Error ? err.message : "WireGuard server unreachable" }
    }
  })
  .post(
    "/peers",
  ...
  .delete("/peers/:username", async ({ params, set }) => {
    ...
    try {
      ...
      set.status = 503
      return { error: "SSH_CONNECTION_FAILED", message: err instanceof Error ? err.message : "Failed to remove peer" }
    }
  })
  .get("/peers/:username/config", async ({ params, set }) => {
    ...
    try {
      ...
      set.status = 503
      return { error: "CONFIG_FETCH_FAILED", message: err instanceof Error ? err.message : "Failed to fetch config" }
    }
  })
  .get("/peers/:username/qr", async ({ params, set }) => {
    ...
    try {
      ...
      if (!base64) { set.status = 500; return { error: "QR_GENERATION_FAILED", message: "Failed to generate QR code" } }
      ...
      set.status = 503
      return { error: "QR_GENERATION_FAILED", message: err instanceof Error ? err.message : "Failed to generate QR" }
    }
  })
