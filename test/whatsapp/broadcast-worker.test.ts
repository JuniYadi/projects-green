...
class QueueMock {
  ...
  async add(..._args: unknown[]) { return {} as unknown }
  async close() {}
}
...
function recipient(overrides: Record<string, unknown> = {}) {
  return {
    id: "r1", broadcastId: "c1", phoneNumber: "+1555", status: "QUEUED",
    attempts: 0, waMessageId: null, lastError: null, dynamicValues: null, name: "T",
...
function campaign(overrides: Record<string, unknown> = {}) {
  return {
    id: "c1", organizationId: "o1", templateName: "t1", templateLanguage: "en",
    templateParams: ["p1"], status: "PROCESSING", total: 1, queued: 1, sent: 0, failed: 0,
    startedAt: new Date(), endedAt: null, createdAt: new Date(), updatedAt: new Date(),
    whatsappContactGroupId: null, whatsappDeviceId: "d1",
    throttleMaxMessages: null, throttlePerMinutes: null,
    ...overrides,
  }
}
...
function device(overrides: Record<string, unknown> = {}) {
  return {
    id: "d1", tokenEncrypted: "tok", whatsappPhoneId: "pid",
    whatsappBusinessAccountId: "waba", ...overrides,
  }
}
...
beforeEach(() => {
  for (const m of [
    sendTemplateMessageMock, fromDeviceMock,
    recipientFU, recipientUpd, recipientGB,
    campaignFU, campaignUpd, rateFU, rateUp, convUp, msgCr,
  ]) {
    m.mockClear()
  }
  ...
  fromDeviceMock.mockResolvedValue({ sendTemplateMessage: sendTemplateMessageMock })
...
function exec(data: Record<string, string>) {
  return capturedProcessor!({
    id: "j", name: "t", attemptsMade: 0, data,
  } as Job<any>)
}
...
describe("dispatch", () => {
  it("sends template via Meta API and creates conversation + message records", async () => {
    ...
    expect(fromDeviceMock).toHaveBeenCalledWith({
      accessToken: "tok", phoneNumberId: "pid", wabaId: "waba",
    })
    expect(sendTemplateMessageMock).toHaveBeenCalledWith(
      expect.objectContaining({
        to: "+1555", templateName: "t1", templateLanguage: "en", fields: ["p1"],
      }),
    )
    expect(convUp).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          organizationId_contactPhone: { organizationId: "o1", contactPhone: "+1555" },
        },
        create: expect.objectContaining({
          organizationId: "o1", contactPhone: "+1555", lastDirection: "OUTBOX",
        }),
      }),
    )
    expect(msgCr).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          direction: "OUTBOX", messageType: "template", waMessageId: "wmid",
        }),
      }),
    )
  ...
  it("marks recipient FAILED on Meta API error", async () => {
    ...
    expect(recipientUpd).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: "FAILED", lastError: "rate limit hit" }),
      }),
    )
  })
  ...
  it("skips sending when recipient is already SENT", async () => {
    recipientFU.mockResolvedValue(
      recipient({ status: "SENT", broadcast: campaign({ whatsappDevice: device() }) }),
    )
    await exec({ campaignId: "c1", recipientId: "r1", method: "dispatch" })
  ...
  it("includes dynamic values in template fields", async () => {
    ...
    await exec({ campaignId: "c1", recipientId: "r1", method: "dispatch" })
    expect(sendTemplateMessageMock).toHaveBeenCalledWith(
      expect.objectContaining({ fields: expect.arrayContaining(["Alice", "Bob"]) }),
    )
  })
})
...
describe("throttle", () => {
  it("allows sending when within limit and no rate state exists", async () => {
    campaignFU.mockResolvedValue(campaign({ whatsappContactGroupId: "g1", throttleMaxMessages: 100, throttlePerMinutes: 5 }))
  ...
  it("defers sending when rate limit is exceeded", async () => {
    campaignFU.mockResolvedValue(campaign({ whatsappContactGroupId: "g1", throttleMaxMessages: 10, throttlePerMinutes: 1 }))
    rateFU.mockResolvedValue({ organizationId: "o1", whatsappContactGroupId: "g1", windowStartAt: new Date(Date.now() - 10_000), messagesSentInWindow: 999 })
    await exec({ campaignId: "c1", recipientId: "r1", method: "throttle" })
    expect(sendTemplateMessageMock).not.toHaveBeenCalled()
  })

  it("sends without throttle when maxMessages is 0", async () => {
    campaignFU.mockResolvedValue(campaign({ whatsappContactGroupId: "g1", throttleMaxMessages: 0, throttlePerMinutes: 0 }))
    await exec({ campaignId: "c1", recipientId: "r1", method: "throttle" })
    expect(sendTemplateMessageMock).toHaveBeenCalled()
  })
...
describe("status-update", () => {
  it("sets COMPLETED when all recipients sent", async () => {
    ...
    expect(campaignUpd).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ status: "COMPLETED", endedAt: expect.any(Date) }) }),
    )
  })

  it("sets COMPLETED_WITH_ERRORS when some fail", async () => {
    recipientGB.mockResolvedValue([{ status: "SENT", _count: { _all: 2 } }, { status: "FAILED", _count: { _all: 1 } }])
    campaignFU.mockResolvedValue(campaign({ total: 3 }))
    await exec({ campaignId: "c1", recipientId: "r1", method: "status-update" })
    expect(campaignUpd).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ status: "COMPLETED_WITH_ERRORS", sent: 2, failed: 1, endedAt: expect.any(Date) }) }),
    )
  })
  ...
  it("keeps PROCESSING while still in progress", async () => {
    ...
    const call = campaignUpd.mock.calls[0][0] as { data: Record<string, unknown> }
