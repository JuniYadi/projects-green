...
>(
  (
    ...
    const handleTabChange = async (tab: "write" | "preview") => {
      ...
      if (tab === "preview") {
        ...
        try {
          ...
          setPreviewHtml(
            "<p class='text-destructive'>Error connecting to server</p>"
          )
        } finally {
          setIsLoading(false)
        }
    ...
    const handleToolbarClick = (
      syntax: "bold" | "italic" | "code" | "link" | "list"
    ) => {
      ...
      switch (syntax) {
        case "bold":
          ...
          selectionOffsetEnd =
            selectionOffsetStart + (selectedText || "bold text").length
          break
        case "italic":
          replacement = `*${selectedText || "italic text"}*`
          selectionOffsetStart += 1
          selectionOffsetEnd =
            selectionOffsetStart + (selectedText || "italic text").length
          break
        case "code":
          if (selectedText.includes("\n")) {
            ...
            selectionOffsetEnd =
              selectionOffsetStart + (selectedText || "code block").length
          } else {
            replacement = `\`${selectedText || "code"}\``
            selectionOffsetStart += 1
            selectionOffsetEnd =
              selectionOffsetStart + (selectedText || "code").length
          }
          break
        case "link":
          ...
          selectionOffsetStart += 1
          selectionOffsetEnd =
            selectionOffsetStart + (selectedText || "link text").length
          break
        case "list":
          replacement = `\n- ${selectedText || "list item"}`
          selectionOffsetStart += 3
          selectionOffsetEnd =
            selectionOffsetStart + (selectedText || "list item").length
          break
      }
    ...
    return (
      <div className="flex w-full flex-col overflow-hidden rounded-lg border border-white/[0.08] bg-neutral-900/20">
        {/* Editor Tabs Header */}
          ...
          {activeTab === "write" && (
            <div className="ml-2 flex items-center gap-1 border-l border-white/[0.08] pl-2">
              <Button
                ...
                disabled={disabled}
              >
                <svg className="h-3.5 w-3.5 fill-current" viewBox="0 0 256 256">
        ...
        <div className="flex-1 bg-transparent p-0.5">
          ...
          {activeTab === "preview" && (
            <div className="max-h-[400px] min-h-[120px] overflow-y-auto rounded-none border-0 bg-neutral-900/10 p-3.5">
              {isLoading ? (
