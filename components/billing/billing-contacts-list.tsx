...
export function BillingContactsList({ orgId }: BillingContactsListProps) {
  ...
  const fetchAccount = useCallback(async () => {
    try {
      // ponytail: if orgId provided, fetch via admin endpoint; use session-based endpoint otherwise
      const result = orgId
        ? await getAdminBillingContacts(orgId)
        : await getBillingAccount()
      setAccount(result as BillingAccountDetail)
      setError(null)
    } catch (err) {
      setError(
