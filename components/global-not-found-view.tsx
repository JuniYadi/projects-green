...
export function GlobalNotFoundView({
  ...
  const t = messages.sharedComponents.notFound
  const copyright = messages.pHomeFooter.copyright

  const isConsole = pathnameWithoutLocale.startsWith("/console")
  const isPortal = pathnameWithoutLocale.startsWith("/portal")
  const targetSurface = isPortal ? "portal" : isConsole ? "console" : "website"

  const fallbackHref = localizePathname({
    pathname: isPortal ? "/portal" : isConsole ? "/console" : "/",
    locale: activeLocale,
  })

  const fallbackLabel = isPortal
    ? `${t.returnTo} Portal`
    : isConsole
      ? `${t.returnTo} Console`
      : t.home
    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <main className="flex flex-1 items-center justify-center p-4 sm:p-8">
        <div className="w-full max-w-lg rounded-2xl border border-border/70 bg-card/80 p-6 text-center shadow-sm backdrop-blur-md sm:p-10">
          <div className="mx-auto mb-5 flex size-14 items-center justify-center rounded-2xl border border-border/80 bg-muted/50 shadow-inner">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-muted-foreground">
            {isPortal || isConsole
              ? t.description.replace("{surface}", targetSurface)
              : t.subDescription}
          </p>

          {pathname && pathname !== "/" && (
            <div className="mt-4 flex items-center justify-center">
              <code className="max-w-xs break-all rounded border border-border/60 bg-muted/50 px-2.5 py-1 font-mono text-xs text-muted-foreground sm:max-w-sm">
                {pathname}
              </code>
            </div>
          )}
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-2.5 sm:flex-row">
            ...
            {!isConsole && !isPortal && (
              ...
              >
                <Link href={localizePathname({ pathname: "/docs", locale: activeLocale })}>
                  {t.docs}
                </Link>
              </Button>
            )}
