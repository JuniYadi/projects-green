...
const isSignupIntent = (intent: string | null) => intent === "signup"
...
export const GET = async (request: NextRequest) => {
  ...
  // getSignInUrl/getSignUpUrl calls setPKCECookie internally via cookies()
  // from next/headers. Route Handlers return a fresh NextResponse that does NOT
  // inherit those queued cookies automatically, so copy them onto the response.
  const authUrl = signupIntent
    ? await getSignUpUrl({ returnTo: next, redirectUri })
    : await getSignInUrl({ returnTo: next, redirectUri })
  ...
  // Forward PKCE verifier cookies set by getSignInUrl/getSignUpUrl
