...
describe("TemplateBuilderPage", () => {
  ...
  it("renders Step 1: General Info by default", () => {
    const { getByText, getByPlaceholderText } = render(<TemplateBuilderPage />)
    expect(getByText("Custom Template Builder")).toBeDefined()
    expect(getByText("Step 1: General Info")).toBeDefined()
    expect(
      getByPlaceholderText("e.g. Next.js High Performance Stack")
    ).toBeDefined()
  })
  ...
  it("navigates through the 4 steps after filling required fields", async () => {
    ...
    const taglineInput = getByPlaceholderText(
      "e.g. Production-ready Next.js 15 app with Redis caching and PostgreSQL"
    )
    const descInput = getByPlaceholderText(/Explain what this stack does/i)
    ...
    await waitFor(() => {
      expect(getByText("Step 2: Container Runtime")).toBeInTheDocument()
    })

    // Step 2 - Fill image
    const imageInput = getByPlaceholderText(/e\.g\. ghcr\.io\/org\/my-app/i)
    ...
    await waitFor(() => {
      expect(getByText("Step 3: Dependencies & Storage")).toBeInTheDocument()
    })
    ...
    await waitFor(() => {
      expect(
        getByText("Step 4: Environment Schema Builder")
      ).toBeInTheDocument()
    })
  })
  ...
  it("allows adding environment variables in Step 4", async () => {
    ...
    await user.type(
      getByPlaceholderText(/Explain what this stack does/i),
      "Description"
    )
    await user.click(getByRole("button", { name: /Next/i }))
    ...
    await waitFor(() =>
      expect(
        getByPlaceholderText(/e\.g\. ghcr\.io\/org\/my-app/i)
      ).toBeInTheDocument()
    )
    await user.type(
      getByPlaceholderText(/e\.g\. ghcr\.io\/org\/my-app/i),
    ...
    // Add variable
    const addVarBtn = getByRole("button", { name: /Add Variable/i })
    await user.click(addVarBtn)
    expect(getByPlaceholderText("e.g. API_KEY or PORT")).toBeInTheDocument()
  })

  it("submits the template successfully as workspace template", async () => {
    const { getByPlaceholderText, getByRole } = render(<TemplateBuilderPage />)
    const user = userEvent.setup({ delay: null })
