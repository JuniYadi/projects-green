...
const mockConversationsGet = mock(() => {
  ...
  return Promise.resolve({
    ...
    conversation: {
      ...
      contactPhone: "+6280000000000",
      lastMessageAt: nullable,
      lastDirection: nullable,
      whatsappDeviceId: nullable,
    ...
    languages: [
      {
        ...
        parameters: {
          components: [
            { type: "BODY", example: { body_text: [["John", "5"]] } },
          ],
        },
        isApproved: true,
        metaStatus: "APPROVED",
...
describe("WhatsAppMessagesPage", () => {
  ...
  it("renders the page with a Send Message button", async () => {
  ...
  it("opens send dialog and shows 2-column layout", async () => {
    ...
    fireEvent.change(view.getByPlaceholderText("+628123456789"), {
      target: { value: "+6289876543210" },
    })
  ...
  it("shows send button enabled after filling all fields", async () => {
    ...
    fireEvent.change(view.getByPlaceholderText("Value for {{1}}"), {
      target: { value: "John" },
    })
    fireEvent.change(view.getByPlaceholderText("Value for {{2}}"), {
  ...
  it("accepts Indonesian local phone 085708296482", async () => {
    ...
    await tick(50)
    const phoneInput = view.getByPlaceholderText(
      "+628123456789"
    ) as HTMLInputElement
  ...
  it("collapses template picker after selection and shows Change Template button", async () => {
    ...
    expect(
      view.getByPlaceholderText("Type to filter templates...")
    ).toBeInTheDocument()
    fireEvent.click(view.getAllByText("hello_world")[0])
    await tick(50)
    expect(
      view.queryByPlaceholderText("Type to filter templates...")
    ).not.toBeInTheDocument()
    expect(
      view.getByRole("button", { name: "Change Template" })
    ).toBeInTheDocument()
  ...
  it("lacks Conversations heading and search has correct aria-label", async () => {
    ...
    expect(view.getByPlaceholderText("Search phone number...")).toHaveAttribute(
      "aria-label",
  ...
  it("shows template preview with filled values when fields are entered", async () => {
    ...
    const field1 = view.getByPlaceholderText(
      "Value for {{1}}"
    ) as HTMLInputElement
  ...
  it("selects a conversation from the phone query parameter", async () => {
    ...
    await tick(50)
    await waitFor(() => {
      expect(mockConversationsGet).toHaveBeenCalledWith("conv_1")
    })
    view.unmount()
  })
  ...
  it("applies bounded-layout CSS classes to thread grid and cards", async () => {
    ...
    expect(
      view.container.querySelectorAll(".flex-1").length
    ).toBeGreaterThanOrEqual(1)
    expect(
      view.container.querySelectorAll(".min-h-0").length
  ...
  it("sends a template and updates URL when send succeeds", async () => {
    ...
    expect(mockRouterReplace).toHaveBeenCalledWith(
      "/en/console/whatsapp/messages?phone=6289876543210",
      { scroll: false }
    )
    ...
    await waitFor(() => {
      expect(mockConversationsGet).toHaveBeenCalledWith("conv_new")
    })
  ...
  it("updates URL even when post-send lookup fails", async () => {
    ...
    // Fill phone
    await user.type(
      view.getByPlaceholderText("+628123456789"),
      "+6289876543210"
    )
    await user.click(view.getAllByText("hello_world")[0])
    await waitFor(() => {
