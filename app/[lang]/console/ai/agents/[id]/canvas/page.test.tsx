...
import AiWorkflowCanvasPage from "./page"

describe("AI Studio Canvas simulator integration", () => {
  afterEach(cleanup)
  ...
  it("hydrates the selected starter template from the URL", async () => {
    ...
    const view = render(<AiWorkflowCanvasPage />)
  ...
  it("renders localized canvas labels", () => {
    const html = renderToString(<AiWorkflowCanvasPage />)
  ...
  it(
    ...
    async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          view.getByRole("button", { name: "Send message" })
        ).toBeDefined()
        expect(
          view.getByRole("button", { name: "Ask for input" })
        ).toBeDefined()
      })
    }
  )
  ...
  it("renders header actions and allows simulation trigger", async () => {
    ...
    await waitFor(() => {
      expect(
        view.getByRole("button", { name: "Simulate test" })
      ).toBeDefined()
      expect(
