...
describe.serial("AddonEditorPage local draft behavior", () => {
  beforeEach(() => {
    localStorage.clear()
    mockAddonQuery.mockClear()
    mockAddonQuery.mockReturnValue(createAddonQueryResult())
  ...
  it.serial(
    "persists archive state and restores it from the local draft after reload",
    async () => {
      ...
      const archiveActions = firstView.getAllByRole("button", {
        name: "Archive",
      })
      ...
      await waitFor(() =>
        expect(reloadedView.getByText("Inactive")).toBeTruthy()
      )
    }
  )
  it.serial(
    ...
    async () => {
      ...
      await waitFor(() =>
        expect(
          view.getByRole("heading", { name: "Draft storage" })
        ).toBeTruthy()
      )
      const user = userEvent.setup()
      const nameInput = view.getByLabelText("Name *")
      await user.clear(nameInput)
      await user.type(nameInput, "Updated storage")
      await waitFor(() =>
        expect(
          view.getByRole("button", { name: "Save draft" })
        ).not.toBeDisabled()
      )
  ...
  it.serial(
    "ignores malformed local drafts and renders the server addon",
    async () => {
      ...
      await waitFor(() =>
        expect(
          view.getByRole("heading", { name: "Extra storage" })
        ).toBeTruthy()
      )
    }
  )
