...
function makeSub(overrides: Partial<VpnSubscriptionItem> = {}): VpnSubscriptionItem {
  return {
    ...
    deviceCount: 2,
    serverAccounts: [
      { id: "sa-1", serverId: "srv-1", serverName: "SG-1", hostname: "sg-1.example.com", ipAddress: "10.0.0.1", region: { name: "Singapore", slug: "sg", countryCode: "SG" }, port: 1194, protocol: "OPENVPN", username: "vpn-org-123-srv-1-openvpn-ab", provisioningStatus: "ACTIVE", failureReason: null, hasConfig: true, hasCredentials: true, createdAt: "2024-01-01T00:00:00.000Z", updatedAt: "2024-01-01T00:01:00.000Z" },
      { id: "sa-2", serverId: "srv-2", serverName: "US-1", hostname: "us-1.example.com", ipAddress: "10.0.0.2", region: { name: "United States", slug: "us", countryCode: "US" }, port: 51820, protocol: "WIREGUARD", username: "vpn-org-123-srv-2-wireguard-cd", provisioningStatus: "FAILED", failureReason: "Connection timeout", hasConfig: false, hasCredentials: false, createdAt: "2024-01-01T00:00:00.000Z", updatedAt: "2024-01-01T00:02:00.000Z" },
    ],
    provisioningSummary: { active: 1, pending: 0, failed: 1, revoked: 0, total: 2 },
    priceLocked: "100000", currency: "IDR", originalPrice: "100000", originalCurrency: "IDR", exchangeRate: 1,
  ...
  it("hides Retry All Failed button when no failed accounts", async () => {
    mockListVpnAdminSubscriptions.mockResolvedValue(
      makeSuccess([makeSub({
        ...
        serverAccounts: [{ id: "sa-1", serverId: "srv-1", serverName: "SG-1", hostname: "sg-1.example.com", ipAddress: "10.0.0.1", region: { name: "Singapore", slug: "sg", countryCode: "SG" }, port: 1194, protocol: "OPENVPN", username: "vpn-org-123-srv-1-openvpn-ab", provisioningStatus: "ACTIVE", failureReason: null, hasConfig: true, hasCredentials: true, createdAt: "2024-01-01T00:00:00.000Z", updatedAt: "2024-01-01T00:01:00.000Z" }],
      })])
    )
    const view = render(<SubscriptionsTable />)
    ...
    await waitFor(() => {
      const hasRetryBtn = view.getAllByRole("button").some(b => b.textContent?.includes("Retry All Failed"))
