...
export const ENDPOINTS = {
  ...
  BUSINESS_PROFILE: (phoneNumberId: string) => getEndpoint(`${phoneNumberId}/whatsapp_business_profile`),
  TEMPLATES: (wabaId: string) => getEndpoint(`${wabaId}/message_templates`),
  WABA_INFO: (wabaId: string) => getEndpoint(wabaId),
  WABA_PHONE_NUMBERS: (wabaId: string) => getEndpoint(`${wabaId}/phone_numbers`),
  WEBHOOK_SUBSCRIPTION: (appId: string) => getEndpoint(`${appId}/subscriptions`),
}
