  ...
  },
  {
    ...
    resources: {
      ...
      provisioningFields: [
        ...
        {
          id: "verification_doc1_type",
          name: "verificationDoc1Type",
        ...
        {
          id: "verification_doc1_url",
          name: "verificationDoc1Url",
          label: "URL File Verifikasi Berkas (1)",
          type: "url",
          placeholder: "https://drive.google.com/... or link file",
          required: true,
        },
        {
          id: "verification_doc2_type",
          name: "verificationDoc2Type",
        ...
        {
          id: "verification_doc2_url",
          name: "verificationDoc2Url",
        ...
        },
        {
          ...
          type: "text",
          placeholder: "Nama sesuai PT (3-25 karakter)",
          required: true,
        },
        {
          id: "avatar_url",
...
const pricings = [
  ...
  {
    planCode: "WHATSAPP_PRIVATESHARE",
    regionCode: "GLOBAL",
    type: "BUNDLE",
    billingMode: "PACKAGE",
    basePriceIdr: 1800000,
    monthlyCapIdr: 1800000,
  ...
  {
    planCode: "WHATSAPP_PRIVATE",
    regionCode: "GLOBAL",
    type: "BUNDLE",
    billingMode: "PACKAGE",
    basePriceIdr: 3000000,
    monthlyCapIdr: 3000000,
    billingPeriod: "QUARTERLY",
    periodPrice: 3000000,
    unitRateCpu: null,
    unitRateMem: null,
...
async function seedRegions() {
  ...
  for (const region of regions) {
    const existing = await prisma.serviceRegion.findUnique({
...
async function seedPlans() {
  ...
  for (const plan of plans) {
    const pkg = await prisma.servicePackage.findUnique({
      where: { code: plan.packageCode as any },
    })
    if (!pkg) {
      console.error(
        `  ⚠️ Package ${plan.packageCode} not found, skipping plan ${plan.code}`
      )
      continue
    }
...
async function seedPricings() {
  ...
  for (const pricing of pricings) {
    ...
    const foundPlan = await prisma.servicePlan.findFirst({
      where: { packageId: pkg.id, code: plCode },
    })
    if (!foundPlan) {
      console.error(
        `  ⚠️ Plan ${plCode} in package ${pkgCode} not found, skipping pricing`
      )
      skipped++
      continue
    }
    ...
    const data: any = {
      ...
      billingPeriod: pricing.billingPeriod ?? undefined,
      periodPrice:
        pricing.periodPrice != null
          ? new Prisma.Decimal(pricing.periodPrice)
          : undefined,
